...
class AuthDebugger {
  ...
  async captureState(stepName) {
    ...
    try {
      ...
      console.log('Could not access storage:', error.message);
    }
  ...
  async addResult(step, success, error) {
    const result = {
  ...
  async testLogin() {
    try {
      ...
      if (!submitButton) {
        submitButton = await this.page.$('button:contains("Login")');
      }
      if (!submitButton) {
        submitButton = await this.page.$('button:contains("Sign in")');
      }
      if (!submitButton) {
